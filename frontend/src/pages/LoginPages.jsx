...
function LoginPage() {
  ...
  const { signin, authErrors, isAuthenticated, user, loading } = useAuth();
  ...
  useEffect(() => {
    if (isAuthenticated) navigate("/finanzas");
  }, [isAuthenticated]);
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center">
          <h1 className="text-2xl font-semibold text-black">
            Ingresa a tu cuenta para llevar el control de tus finanzas
            personales
          </h1>
        </div>
        ...
        >
          ...
          <div className="flex flex-col text-left">
            ...
            <input
              ...
              className={`w-full bg-gray-200 text-gray-900 px-4 py-2 rounded-md outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300 ${
        ...
        <footer className="mt-8 bg-gray-50 text-center text-sm text-gray-500 py-4 border-t border-gray-200">
          <p className="text-xs">
            © {new Date().getFullYear()} FinanzasApp. Todos los derechos
            reservados.
          </p>
        </footer>
      </div>
      {/* Modal ventana loading */}
      {loading && (
        <div className="fixed inset-0 bg-white/80 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-8 w-full max-w-sm flex flex-col items-center">
            ...
            <p className="text-sm text-gray-600 mt-1">Por favor espera</p>
          </div>
        </div>
      )}
    </div>
